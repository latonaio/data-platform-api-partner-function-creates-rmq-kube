...
import (
	dpfm_api_caller "data-platform-api-partner-function-creates-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	errs := caller.AsyncPartnerFunctionCreates(accepter, &input, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"PartnerFunction", "PartnerFunctionText",
		}
	}
	return accepter
}
